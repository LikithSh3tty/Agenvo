...
Hi — I'm **Likith Shetty**, a developer who likes building complete products, not just features. I'm drawn to the full arc of an app: the data model, the API in the middle, and the interface a real person actually touches. agencyx is that instinct in one place — a genuinely useful tool with a considered UI, a secure per-user data layer, and an AI assistant that's careful about what it claims.

I'm especially interested in **LLM-powered apps that stay honest** — systems where the model is a great communicator but the facts come from code you can trust. Both agencyx and my CloudNest support agent are built on that idea: let the language model handle the language, and keep the truth deterministic.

Mostly I work across React, Python, and the Anthropic and Firebase ecosystems, and I care about the details — smooth interactions, clear copy, and code that's easy to come back to.

Reach me at **shettylikith.rajesh@msam.christuniversity.in**.
