...
// east of UTC during their morning (in IST, midnight-05:30 reported yesterday),
// which mis-dated new sales and made the Today filter miss them.
...
//
// The number is a real running sequence, not a hash of a record id. Splitting the
// template around {SEQ} yields the series "skeleton" (for "INV/{FY}/{SEQ}" that is
// the prefix "INV/26-27/" and an empty suffix); the highest sequence already
// issued under that same skeleton, plus one, is the next number. A different
// fiscal year or month bucket forms its own series and restarts at 0001.
// Callers must store the returned number on the invoice - it is what makes the
// invoice a stable, identifiable document.
