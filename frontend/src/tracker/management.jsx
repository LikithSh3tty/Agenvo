...
export function InvoicesPanel({ invoices = [], onUpsert, onOpen, onDelete, delay = 0 }) {
  ...
  if (!list.length) {
    return <EmptyState icon="file-text" text="No invoices yet" sub="Pick sales in History and choose Create invoice. Every invoice you download or email is filed here automatically." />;
  }
  const cols = "1.1fr 1fr 0.9fr 0.8fr 0.8fr 92px 64px";
  return (
    <div>
          ...
          return (
            <div key={inv.id || inv.number} className="recrow" style={{ display: "grid", gridTemplateColumns: cols, minWidth: 700, padding: "12px 18px", borderTop: "1px solid rgba(var(--accent-rgb),0.03)", fontSize: 13, alignItems: "center", gap: 6 }}>
              ...
              <div style={{ whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                <span style={{ fontWeight: 600 }}>{inv.clientName}</span>
                {lines > 1 && <span style={{ color: C.textMuted, fontSize: 11, marginLeft: 6, fontFamily: "'JetBrains Mono',monospace" }}>{lines} sales</span>}
              </div>
...
export function ManagementApp({ data, persist, config, onSettings, onInvoice, onExport, onImport }) {
  ...
  // Matched on id, not number: number-keyed upserts used to overwrite earlier
  // invoices whenever two numbers collided. The number branch is legacy-only.
